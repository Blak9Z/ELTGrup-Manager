...
export async function seedChecklistTemplates() {
  ...
  const data: Prisma.ChecklistTemplateCreateManyInput[] = defaultChecklistTemplates.map((t, i) => ({
    name: t.name,
    category: t.category,
    items: t.items,
    projectType: t.projectType ? "RESIDENTIAL" : undefined,
    sortOrder: i,
  }));

  const result = await prisma.checklistTemplate.createMany({ data, skipDuplicates: true });
  return { count: result.count, seeded: true };
}
